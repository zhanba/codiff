...
import { assert, assertFn, checkAdjacentItems } from "./assert.js";
...
export class TextEdit {
  ...
  mapPosition(position: Position): Position | Range {
    ...
    for (const edit of this.edits) {
      ...
      lineDelta +=
        len.lineCount - (edit.range.endLineNumber - edit.range.startLineNumber);
  ...
  inverse(doc: AbstractText): TextEdit {
    ...
    return new TextEdit(
      this.edits.map(
        (e, idx) =>
          new SingleTextEdit(ranges[idx], doc.getValueOfRange(e.range)),
      ),
    );
  }
